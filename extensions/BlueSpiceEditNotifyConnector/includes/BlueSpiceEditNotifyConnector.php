...
class BlueSpiceEditNotifyConnector {
	 ...
	 */
	static $arrActions = [ // labels will not be used
		"create" => "Creation",
	...
	public static function onGetPreferences( User $user, array &$preferences ) {
		...
		$preferences[ 'notify-namespace-selection' ] = [
			'type' => 'checkmatrix',
	 ...
	 * @param User $user
	 * @param array &$options
	 * @return bool handover to next call
	 */
	public static function onUserSaveOptions( User $user, array &$options ) {
		...
		$dbw->delete( self::$tableName, [ "enc_username" => $user->getName() ] );
		...
		foreach ( self::$arrActions as $oKey => $oVal ) {
			...
			try {
				...
				$res = $dbr->select(
						[ self::$tableName ],
						[ 'enc_username' ],
						[ 'enc_ns_id' => $nsID, 'enc_action' => $oKey ],
						__METHOD__
				);
				$arrUser = [];
				foreach ( $res as $row ) {
